...
/* ─────────────────────────────────────────────────────────────────
   EMAILJS CONFIGURATION — via Next.js public env vars
   ──────────────────────────────────────────────────────────────── */
...
const EMPTY: FormData = { name: "", email: "", phone: "", company: "", message: "" };
...
export function ContactModal({ open, onClose, lang }: ContactModalProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      const t = setTimeout(() => { setData(EMPTY); setErrors({}); setStatus("idle"); }, 300);
  ...
  const validate = (): FormErrors => {
    const e: FormErrors = {};
    if (!data.name.trim()) e.name = l.required;
    if (!data.email.trim()) e.email = l.required;
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email)) e.email = l.emailInvalid;
    if (!data.company.trim()) e.company = l.required;
    if (!data.message.trim()) e.message = l.required;
    return e;
  };
            ...
            <form onSubmit={onSubmit} noValidate>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
                ...
                <div>
                  ...
                  />
                  {errors.name && <span style={errBase}>{errors.name}</span>}
                </div>
                {/* Email */}
                <div>
                  ...
                  />
                  {errors.email && <span style={errBase}>{errors.email}</span>}
                </div>
                {/* Phone */}
                <div>
                ...
                <div>
                  ...
                  />
                  {errors.company && <span style={errBase}>{errors.company}</span>}
                </div>
              </div>
              {/* Message */}
              <div style={{ marginBottom: 24 }}>
                ...
                />
                {errors.message && <span style={errBase}>{errors.message}</span>}
              </div>
              {/* Submit */}
              <button
                type="submit"
